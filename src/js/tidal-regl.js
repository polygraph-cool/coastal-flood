...
/*
function load(err, regl) {
  
}

function constructScene(regl) {

  const nPoints = sumArray(Object.values(floodedByYear));
  const pointWidth = 4;
  const pointMargin = 1;
  const width = $wrap.getBoundingClientRect().width;
  const height = $wrap.getBoundingClientRect().width;
  ...
  const toMap = points => mapLayout(points, pointWidth + pointMargin, width / 2, height / 2);
  const toKtGrid = points => ktGridLayout(points, pointWidth + pointMargin, width);

  const layouts = [toMap, toKtGrid];
